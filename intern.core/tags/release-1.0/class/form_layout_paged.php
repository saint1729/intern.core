...
/*
 *
 * @(#) $Id: form_layout_paged.php,v 1.20 2008/07/19 07:54:40 mlemos Exp $
 *
 */
...
class form_layout_paged_class extends form_custom_class
{
	...
	Function AddInputPart(&$form)
	{
		...
		|| strlen($error = $form->AddDataPart($row_start)))
			return($error);
		$t = count($this->pages);
		for(Reset($this->pages), $p = 0; $p < $t; Next($this->pages), ++$p)
		{
	...
	Function PageHead(&$form)
	{
		...
		$head = '<script type="text/javascript"><!--'.$eol.
			...
			' {'.$eol.
			'  if(width)'.$eol.
			'   parent.style.width = width'.$eol.
			'  if(height)'.$eol.
			'   parent.style.height = height'.$eol.
			' }'.$eol.
			'}'.$eol
			: '').
			'// --></script>'.$eol;
