/*
 *
 * @(#) $Id: animation.js,v 1.16 2008/09/07 06:24:27 mlemos Exp $
 *
 */
	...
	var stepEffects = function()
	{
		for(var a = 0; a<ML.Animation.animations.length; a++)
		{
			...
			&& ML.Animation.animations[a].running)
			{
				...
				if(effect.type == 'FadeOut')
				{
					...
					if(!step)
					{
						if(effect.visibility == 'display')
							effect.node.style.display = 'none'
						else
				...
				if(effect.type == 'Wait')
				{
					step=(new Date().getTime() - effect.start < effect.duration*1000.0)
				}
				}
				}
				if(!step)
	...
	var startEffect = function(animation)
	{
		...
		if(type == 'FadeIn')
		{
			...
			if(effect.node)
			{
				...
				if(effect.visibility == 'display')
					effect.node.style.display = 'block'
				else
